...
export default function UnkanApp() {
  ...
  useEffect(() => {
    ...
    }, event.planningMode === "schedule" ? 20 : (reducedMotion ? 20 : 1700));
    return () => window.clearTimeout(timer);
  }, [event, member?.role, reducedMotion]);
...
function ResultStage({ event, reducedMotion, soundEnabled, meta, onReact }: { event: EventData; reducedMotion: boolean; soundEnabled: boolean; meta: EventMeta; onReact: (reaction: string) => void }) {
  ...
  useEffect(() => { const timers = [window.setTimeout(() => setStep("counting"), reducedMotion ? 1 : 800), window.setTimeout(() => setStep("eliminating"), reducedMotion ? 2 : 1700), window.setTimeout(() => setStep("vader"), reducedMotion ? 3 : 2350), window.setTimeout(() => setStep("winner"), reducedMotion ? 4 : 3300)]; return () => timers.forEach(window.clearTimeout); }, [reducedMotion]);
